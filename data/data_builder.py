...
from data import PairedTextDataset
from data import (
    load_glove,
    PairedTextDataset,
)
...
def load_and_prepare_bug_data(reponame: str, repodir: str, split_ratio: float = 0.33):
    ...
    train_paired_data, train_weights = build_paired_data(train_bugs, indexing_dir, brid2commit)
    val_paired_data, val_weights = build_paired_data(val_bugs, indexing_dir, brid2commit)

    return train_paired_data, train_weights, val_paired_data, val_weights

# === Function to build vocab and DataLoaders ===
def prepare_dataloaders(train_paired_data, val_paired_data, batch_size=32, embedding_dim=100):
    ...
    train_dataset = PairedTextDataset(train_paired_data, vocab, tokenizer, max_length=max_len)
    val_dataset = PairedTextDataset(val_paired_data, vocab, tokenizer, max_length=max_len)
...
def paired_collate_fn(batch, pad_idx):
    text1_batch, text2_batch, label_batch = zip(*batch)
