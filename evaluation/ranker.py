...
def evaluate_bug_ranking(model, reponame, repodir, max_length=4102, glove_path='artifacts/glove6B', embedding_dim = 100, device=None):
    ...
    test_paired_data, _ = build_paired_data(test_bugs, test_indexing_dir, brid2commit=None)  # brid2commit unused for test bugs
    ...
    for bug in test_bugs:
        brid = bug['number']
        paired_data, _ = build_paired_data([bug], test_indexing_dir, brid2commit=None)
